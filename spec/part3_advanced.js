  ...
  describe('Part III --- ADVANCED', function() {
    ...
    describe('TEST ONLY: Object Values', function() {
      ...
      it('TEST ONLY: Access object values using bracket notation ', function() {
        ...
        expect(_.objectCreationBracket(obj["firstName"])).to.eql('Tommy');
        expect(_.objectCreationBracket(obj["lastName"])).to.eql('Oliver');
        expect(_.objectCreationBracket(obj["age"])).to.eql(16);
      });

      it('TEST ONLY: Access object values using dot notation ', function() {
        //Use bracket notation for the FILL_ME_IN portion.
        expect(_.objectCreationDot(obj.zord)).to.eql('Dragon');
        expect(_.objectCreationDot(obj.suit)).to.eql('Green');
        expect(_.objectCreationDot(obj.inHighSchool)).to.eql(true);
      });
    ...
    describe('Object Creation using Bracket Notation', function() {
      ...
      let lastName = 'Oliver';
      let zord = 'Dragon';
      let obj = _.objectCreationBracket1(firstName,lastName,zord);
    ...
    describe('Object Creation using Dot Notation', function() {
      ...
      let lastName = 'Oliver';
      let zord = 'Dragon';
      let obj = _.objectCreationDot1(firstName,lastName,zord);
    ...
    describe('TEST ONLY: Array Of Objects', function(){
      ...
      it('TEST ONLY: Access the value needed to pass the test', function(){
        expect(_.arrOfObjs(rangers[3])).to.eql({firstName : 'Billy',lastName : 'Cranston',
        suit : 'Blue'});
        expect(_.arrOfObjs(rangers[4])).to.eql({firstName : 'Zack',
        lastName : 'Taylor',
        suit : 'Black'});
        expect(_.arrOfObjs(rangers[0].firstName)).to.eql("Tommy");
        expect(_.arrOfObjs(rangers[4].suit)).to.eql('Black');
        expect(_.arrOfObjs(rangers[2].lastName)).to.eql("Kwan");
        expect(_.arrOfObjs(rangers[1].suit)).to.eql('Red');
        expect(_.arrOfObjs(rangers[5].lastName)).to.eql("Ann Hart");
        expect(_.arrOfObjs(rangers[3].firstName)).to.eql('Billy');
      });
    });
    ...
    describe('TEST ONLY: Object Of Objects', function(){
      ...
      it('TEST ONLY: Access the value needed to pass the test', function(){
        //Use dot notation
        expect(_.objOfObj(rangers.blue)).to.eql({firstName : 'Zack',
        lastName : 'Taylor',
        zord : 'Mastodon'});
        //Use bracket notation
        expect(_.objOfObj(rangers[1])).to.eql({firstName : 'Jason',
        lastName : 'Lee',
        zord : 'Tyrannosaurus'});
        //Use dot notation
        expect(_.objOfObj(rangers.yellow.firstName)).to.eql.to.eql('Trini');
        expect(_.objOfObj(rangers.green.zord)).to.eql.to.eql('Dragon');
        expect(_.objOfObj(rangers.blue.lastName)).to.eql('Cranston');
        //Use bracket notation
        expect(_.objOfObj(rangers["pink"]["lastName"])).to.eql('Ann Hart');
        expect(_.objOfObj(rangers["red"]["zord"])).to.eql('Tyrannosaurus');
        expect(_.objOfObj(rangers["black"]["firstName"])).to.eql('Zack');
        //Use a combination of dot and bracket notation to pass each test
        expect(_.objOfObj(rangers.blue["lastName"])).to.eql('Oliver');
        expect(_.objOfObj(rangers.yellow["zord"])).to.eql('Sabertooth Tiger');
      });
    });
